...
@section('container')
    ...
    @if (!empty($penilaian_gagasan_kreatifs))
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Penilaian Prestasi Gagasan Kreatif</h3>
                    </div>
                    
                    <form action="{{ url('/dashboard/prestasi/penilaian/gagasan-kreatif/'.$prestasi->slug) }}" method="POST">
                        @method('put')
                        @csrf
                        ...
                        <div class="card-body">
                            ...
                            <h4>Rubrik Penilaian Naskah Gagasan Kreatif</h4>
                                    ...
                                    <tbody>
                                        @foreach ($gagasan_kreatifs as $gk)
                                            <tr>
                                                ...
                                                <td class="align-middle">{{ $gk->rubrik_gagasan_kreatif[0]->nilai_min .' <= Skor < '. $gk->rubrik_gagasan_kreatif[0]->nilai_max }}</td>
                                                <td class="align-middle">{{ $gk->rubrik_gagasan_kreatif[0]->rubrik }}</td>
                                                <td rowspan="5" class="align-middle">
                                                    ...
                                                    <input type="text" name="{{ 'skor'.$gk->id }}" class="form-control @error('skor'.$gk->id) is-invalid @enderror" value="{{ old('skor'.$gk->id, $penilaian_gagasan_kreatifs[$loop->index]->skor) }}" placeholder="Skor" required>
                                                    @error('skor'.$gk->id)
                                            ...
                                            @for ($i = 1; $i < 5; $i++)
                                                <tr>
                                                    <td class="align-middle">
                                                        @if ($gk->rubrik_gagasan_kreatif[$i]->nilai_min == 9)
                                                            {{ $gk->rubrik_gagasan_kreatif[$i]->nilai_min .' <= Skor <= '. $gk->rubrik_gagasan_kreatif[$i]->nilai_max }}
                                                        @else
                                                            {{ $gk->rubrik_gagasan_kreatif[$i]->nilai_min .' <= Skor < '. $gk->rubrik_gagasan_kreatif[$i]->nilai_max }}
                                                        @endif
                                                    </td>
                                                    <td class="align-middle">{{ $gk->rubrik_gagasan_kreatif[$i]->rubrik }}</td>
                                                </tr>
                                            @endfor
                                        @endforeach
                                    </tbody>
